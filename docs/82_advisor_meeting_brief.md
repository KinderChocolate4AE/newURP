...
**오늘 기준 확정된 4대 결과**:
1. **운동학 경계** a\* = 2ρ/τ² = 39.3 m/s² — 경계 위 포획 0/1,635(T0) · 0/1,598(T1) [실측]
...
|---|---|---|---|---|
| (a) | **χ>1 운동학 불성립** (a≥39.3 → 포획 불가) | ANALYTIC + MEASURED (0/1,635 T0 · 0/1,598 T1) | "χ는 지배 좌표 **중 하나**" (Gate 10이 단일 좌표 충분성 기각) | 명제 N이 스케치 수준·미비준; τ 하한·ρ 상한이 같은 방향으로 경계 이동 (scope 문장 존재) |
...
| (b′) | **modality gap** — 요격 가능 ≠ 포획 가능 | MEASURED (동결 아티팩트 재분석) | "일반 요격 능력이 남아 있는 구간에서 비파괴 포획만 먼저 붕괴" | 현재 24.3%는 경쟁위험 때문에 **하한** — 전용 HK baseline(E2)이 확정치를 줌 |
