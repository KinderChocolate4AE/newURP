# 13 — A-3 설계: L-reverse 후진 커리큘럼 (v0.1 초안, 2026-07-14) — 비준 대기
...
- [ ] **R-1** T0 재구성·재검증 절차 + 탈락 규칙 (§1)
- [ ] **R-2** scratch 기본 + warm 참고런(선택 금지) (§3)
- [ ] **R-3** R-스테이지 파라미터 (σ·Δx·exit 문턱) (§2)
- [ ] **R-4** 파일럿 중간 게이트 (i)(ii)(iii) + (i) 실패 시 L-2stage 신호 해석 (§4)
- [ ] **R-5** "보상 스캐폴드 0, 스폰 분포만" 원칙 + eval 스폰 frozen STRICT lock (§0·§6)
