# 21 — A-3e 사전등록 패키지 (v0.3.2, 2026-07-18 — 동결본 + §7 분기형 개정(v0.3.1) + **sealed 진단 arm 추가(v0.3.2; 판정·소진 규칙 불변)**)
...
>
> **불변**: 판정 J·게이트 정의·평가 경로·σ 램프 / bank v2 재생성 금지(소진) / **rewind-v2 1회 생성 원칙(신설)** / sealed 불가침 / 문턱 사후 조정 금지 / **신규 실험 착수 마감 2026-08-15**(v0.3.1) / 하드 스톱 **2026-08-31**. **B의 정의(재수록)**: active curriculum 개발을 종료하고, synthetic·on-manifold 전임자의 σ-강건 도달성 실패를 중심 증거로 하는 failure-mode/진단 프레이밍으로 전환하는 노선(docs/12 트립와이어 조항).
...
- **sealed 진단 arm(v0.3.2 추가 — 결과 판독 전 개정)**: 동일 1회 소진 내에서 **brake·lam20 arm을 d1 동일 episode에 실행·기록**(정책-무관 결정론 arm; **판정 규칙 불참**, 소진 규칙 불변). 목적 = **RL-필요성 병목 계측**(docs/20 §6 제4병목): 정책이 zero만 이기고 단순 컨트롤러(validation 실측 d1 brake .85/.79·lam20 .89/.84) 아래인 시나리오를 판독 가능하게 — 학습 가능성 증거와 RL-방법 필요성 증거의 분리 보고 의무.
...
## 7. [E-6] 중단·전환 규칙 (9종; v0.3.1 분기형 개정 — Hyunjun 지적·비준 2026-07-18)

**불변(무결성 조항 — 개정 대상 아님)**: 실패한 실험의 재선택·문턱 조정·재생성 구제 금지 / **⑤ L1 미달 = fire unfreeze 없이 즉시 FAIL(사유: limiter shaping)** / **⑥ 복원·RT-1·RT-2 실패 = "측정기 구현 실패" 기록·중단(가설 기각과 분리; 게인 조정 금지)** / **⑦ source diversity < 2/k = 해당 k 결측** / **⑧ sealed 1회 소진(재선택·재평가·추가 fine-tune 금지)** / **⑨ k=2 multiplicity = pooled 규칙 외 판정 금지** / 각 발동 = docs/12 §6 증거 행(사유 구분 기록).

**개정(사후 라우팅 — 구 "→ 즉시 B"를 교체)**: ① P1′ FAIL ② 수확 전 k 결측 ③ rewind pooled 기각 — 각각 **해당 실험 종결 + 증거 행 기록 후 분기 결정**(docs/20 패턴): (a) **새 사전등록 실험 설계**(0-e급 절차, 실패 모드 판독에 기반한 신규 설계 — 종결된 실험의 구제 아님) 또는 (b) **B 전환** — Hyunjun 비준으로 선택. 근거: 무결성이 요구하는 것은 "그 실험의 구제 금지"까지이며 노선 포기가 아님; 잔여 기간이 허용하는 한 실패 모드별 신규 설계는 정당(각 실패는 어느 분기든 B의 증거 사슬을 강화).

**일정 상한**: **신규 실험 착수 마감 = 2026-08-15**(이후 신규 사전등록 착수 금지 — 잔여 기간은 B 집필·재현 패키징 전용) / **하드 스톱·트립와이어 2026-08-31 불변**(그 시점 도달 단계 무관 B 확정). **B 준비 병행**(docs/20 C 조항 편입): 아웃라인·그림 목록·docs/12 §6 테이블 유지는 실험과 무관하게 지금부터 진행 — 전환 비용 최소화가 실험 여유를 늘림. B의 정의는 서두 재수록분 그대로.
