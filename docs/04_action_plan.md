...
- **자산**: WarSim 3DOF · E_req(→per-shot value) · pointing/action-space 결정 · harness. 정본 = newURP_*; 구 docs = LEGACY.
...
- [ ] 정직 보고(shaping 안 통하는 regime 포함). **DoD = frontier가 *움직임*(capture↑ 아님)**
...
MARL(MAPPO/HAPPO) 구현 · multi-agent reward/credit(COMA) · 학습/평가 규율 · 3DOF 동역학 sim.
+ OPTIONAL **physical-AI stretch**: sim-to-real 한 발 / LiCS 랩 PX4-ROS2-Gazebo (URP 본체는 3DOF 유지) — 랩·진로 정합.
