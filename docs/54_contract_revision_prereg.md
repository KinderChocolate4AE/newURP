...
- **적분기 확정의 귀결**: backend 의 step 변위는 `v_new` 선형 -- **chord 가
  ...
  평활화" 대안이다. backend 는 step 내부의 유일한 연속 궤적을 정의하지 않으므로
  (endpoint map 만 정의) **어느 보간도 자동으로 참값이 아니다** (오류 9). 둘의
  차이(실측 최대 ±0.076)가 보간 모호성의 크기다.
- **결과 (리뷰 3 후속 정정 반영 — "false positive" 표현 폐기)**: 평가한 두
  step-interpolation 규약(chord / 이차 평활화) 사이에서 **contact 판정 불일치
  1/17 (ep19**: chord 0.731 안 / quadratic 0.758 밖, 경계 ±0.01**)**. 나머지
  **16/17 은 두 규약 모두에서 contact 로 판정 일치**. 이것은 **실제 연속 물리
  접촉에 대한 인증이 아니다** — chord-swept 는 현행 이산 환경의 *선택된 event
  contract* 로 정당화되고, endpoint-only 보다 step 내부 관통을 표현한다는
  점에서 낫다는 것까지만 말한다. ep19 는 경계 사례로 별도 표시.
- **"철회 조건 3건" 의 지위 강등**: 이 기준은 세션 내에서 실행 전에 작성됐으나
  결과와 **같은 커밋**(9216fd1)에 들어가 커밋 시각으로 사전등록을 증명할 수
  없다. 따라서 PASS/FAIL 게이트로 쓰지 않고 **민감도 결과**(불일치 1/17)로만
  보고한다.
- chord 오차 상한 0.033 논법은 **철회한다** -- 그 유도는 "정확한 이차 궤적의
  끝점" 가정에 의존하는데 적분기가 그 가정을 만족하지 않는다 (리뷰 3 §3 의
  조건 1 위반 확인). 대체 근거 = 위의 두-규약 직접 비교.
...
- (i) **PASS** 전 구간 -- event 수준 kill 빈도가 Pk 이항 CI95 안 (구현 sanity.
  ...
  episode 판.
- (ii) 재시도 효과 (리뷰 3 후속 정정 — 일반식 표현 금지): **평가한 중간 Pk
  조건(0.25/0.5/0.75)에서** episode-level neutralization 이 반복 contact
  opportunity 때문에 단일-event Pk 보다 약 0.12–0.13 높게 **관측**됐다. 이
  차이는 Pk 에 독립적인 상수가 아니며 Pk=1 에서는 1.000 으로 **포화**된다.
- **재시도의 정체 (계약 확인)**: 같은 limiter 의 PK_FAIL 후 재시도는 **불가**
  하다 — PK_FAIL 은 소모·retire (docs/54 §1 R1: 커밋과 동일). 같은 limiter 의
  재시도는 **veto 후에만** 가능하다 (미소모·재평가, 이번 sweep 에선 veto 0).
  따라서 관측된 재시도 효과는 전부 **서로 다른 limiter 의 순차 접촉**이다.
- **이 sweep 의 이름은 "resolver sanity + 임무 민감도 분석"이다.** Bernoulli
  resolver 가 설정 Pk 를 통계적으로 재현하고 반복 접촉이 episode 성공률을
  올린다는 것까지 지지하며, **실제 접촉 lethality 가 이 곡선을 따른다는 것은
  지지하지 않는다** (physical lethality calibration 아님). 평가 범위 = boxed
  17판·intercept·A2.

### 3.3 §3 종합 문장 (리뷰 3 확정 표현 — 보고서·논문에 이대로 쓴다)

> V2 의 사전등록 수치 재현은 실패했으나, 차이는 사전등록된 분해에서 swept
> contact 검출로 사건 단위 귀속됐다. backend 는 step 내부의 유일한 연속
> 궤적을 제공하지 않으므로 chord-swept 를 실제 물리 궤적의 참값으로 해석하지
> 않으며, 평가한 두 interpolation 규약에서는 16/17 사건의 접촉 판정이
> 일치했다. Pk sweep 은 resolver 의 Bernoulli 의미론과 반복 contact 효과를
> 검증했지만, contact lethality 의 물리적 타당성은 여전히 미검증이다.
