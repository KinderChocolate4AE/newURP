...
`a_att` 를 연속축으로 두고 판을 묶으면 그게 보이고, 실제로 보였다 (docs/45 §9):
    ...
    붕괴 50% 교차 = 22.45        a* = 2 rho / tau^2 = 39.33   (T1 n=2700)
    slew_audit 이 잰 잔여 조준각 psi = 4.26 deg 를 일반형에 넣으면 25.75
    -> 두 값이 12.4 % 안에서 만난다 -- **"consistent with" 까지만** 이다.

    ★ 금지 (registry C031): "두 독립 계측 일치로 검증" / "finite slew binds
      first". psi 의 원인은 미해결이고 slew cap 은 paired 반사실(0/500 flip)
      로 배제됐다. 게다가 두 값은 lineage 가 다르다 -- psi 는 no-fire audit
      world, 곡선은 ratified fire world 산물이다 (docs/83 §12A).
      옛 문구 "6.6 % ... 두 번째 경계의 검증" 은 24.06(legacy) 기준이었고
      인과 귀속까지 주장했다 -- 둘 다 철회됐다 (감사 Session 3 C-008).
