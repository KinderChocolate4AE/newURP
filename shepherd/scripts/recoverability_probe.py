...
class _Driver:
    ...
    def __init__(self, env, scn, lay, ep: int):
        ...
        self.min_swept: float = np.inf    # 개입 후 진단용 (proxy L4)
    ...
    def step(self, limiter_override: Optional[Dict[str, np.ndarray]] = None,
        ...
        for i in range(len(lims2)):
            ...
            self.min_swept = min(self.min_swept, d)
...
def _rollout(src: _Driver, s0: int, plan, reuse: _Driver) -> tuple:
...
def plan_cem(src: _Driver, ep: int, s0: int, a_max: float) -> PlanResult:
    """§7.1: CEM P=64·I=2·elite16 × seed 3. 구조 후보 = intercept·hold."""
    ...
    for ss in SOLVER_SEEDS:
        ...
        for it in range(ITERS):
            ...
            scored = []
            for c in cands:
                scored.append((_rollout(src, s0, c, reuse), c))
                rollouts += 1
...
def run_arm(ep: int, arm: str, t_star: int, s0: Optional[int] = None) -> dict:
    ...
    if arm.endswith("ORC"):
        ...
        pr = plan_cem(src_full, ep, s0, a_max)
        plan = pr.plan
